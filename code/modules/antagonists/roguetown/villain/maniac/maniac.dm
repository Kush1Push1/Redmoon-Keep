...
/datum/antagonist/maniac/on_gain()
	...
	if(owner.current)
		if(ishuman(owner.current))
			...
			dreamer.change_stat("strenght", 16 - dreamer.ROUNDSTART_STASTR, "maniac_role") // REDMOON ADD START - after_death_stats_fix - изменение статов роли по новой системе распределения
			dreamer.change_stat("constitution", 16 - dreamer.ROUNDSTART_STACON, "maniac_role")
			dreamer.change_stat("endurance", 16 - dreamer.ROUNDSTART_STAEND, "maniac_role") // REDMOON ADD END
...
/datum/antagonist/maniac/on_removal()
	...
	if(owner.current)
		...
		if(ishuman(owner.current))
			var/mob/living/carbon/human/dreamer = owner.current
			dreamer.change_stat("strenght", 0, "maniac_role") // REDMOON ADD START - after_death_stats_fix - изменение статов роли по новой системе распределения
			dreamer.change_stat("constitution", 0, "maniac_role") // REDMOON ADD START - after_death_stats_fix - изменение статов роли по новой системе распределения
			dreamer.change_stat("endurance", 0, "maniac_role") // REDMOON ADD START - after_death_stats_fix - изменение статов роли по новой системе распределения
			/* REDMOON REMOVAL START - after_death_stats_fix - для изменения статов используются функции выше
